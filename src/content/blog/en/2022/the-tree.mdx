...
categories:
  - 'Data Structures'
locale: 'en'
ogImage: '/img/hierarchical-structure.png'
---
...
![A tree with a root node and branching child nodes](~/assets/tree.png)
...
There are multiple ways to traverse a tree, but let's look at the two most common ones: DFS (Depth-first) and BFS (Breadth-first).
...
This generator function traverse the tree recursivingly iteraterating over every node of a given subtree of the tree, yielding _*("giving")*_ each one back
to the function caller, before moving to each node children until it has yielded every leaf of the subtree. It then moves to the next one until there are no more.
...
The BFS one, on the other hand, traverse the tree one level at a time, yielding each node of a given level before moving to the next one.

You can use either one depending on your use case.
