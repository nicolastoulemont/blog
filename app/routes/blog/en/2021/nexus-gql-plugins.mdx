...
snippet: 'Buiding typesafe field level plugins to run field level "middlewares" with the graphql nexus framework.'
...
<PostIntro publishedAt="Apr 19th 2021" category="GraphQL" />
...
export const OperationLoggerPlugin = plugin({
  ...
  onCreateFieldResolver() {
    ...
    return async (root, args, ctx, info, next) => {
      ...
      console.log(`${info.operation.operation} ${info.operation.name.value} took ${endTimeMs - startTimeMs} ms`)
      return value
    }
  },
})
...
export const createUser = mutationField("createUser", {
  ...
  async resolve(_, args, context) {
    if (!context.ctx.logged) return { code: "UNAUTHORIZED", message: "UNAUTHENTICATED_PLEASE_LOGIN" }
    // else create user
  },
})
```
...
export const createUser = mutationField("createUser", {
  ...
  authorization: (context) =>
    !context.ctx.logged ? { code: "UNAUTHORIZED", message: "UNAUTHENTICATED_PLEASE_LOGIN" } : undefined,
  async resolve(_, args, context) {
