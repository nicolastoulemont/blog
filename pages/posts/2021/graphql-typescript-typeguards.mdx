...
This usually forces the developer into asserting the response type quite often which can cause a bit of noise. To handle these assertions we will have a look at a few helpul typeguards functions:
isType, isEither, isNot, isTypeInTuple.

## Simple case

For example, when asserting the result of the following mutation response, the developer will need to handle three differents cases: an ActiveUser, a UserAuthenticationError and a InvalidArgumentsError.
...
This isEither function works kind of the same as the isType function except that the runtime iterate on the given typenames.
The type assertion is based on the `result is Extract<Result, { __typename: typeof typenames[number] }>` which assert that the result is one of any of the index values of the typenames array.
...
const activeUsers = useMemo(() => data?.users?.filter((user) => isType(user, 'ActiveUser')) ?? [], [
	data
])
```

One could think that the previous filtering is enough to get the correct users and at runtime and it is. But sadly Typescript doesn't understand that now activeUsers is an array ActiveUsers only.
...
And we will get the a correctly typed ActiveUser array.
