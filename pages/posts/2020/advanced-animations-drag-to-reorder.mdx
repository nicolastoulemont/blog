...
description: 'Learning advanced web animations with Framer Motion and React in order to pêrform drag to reorder operations with fluid animation'
layout: PageLayout
---
...
-   A moveItem function which will find the correct index in the list items array for the dragged items based on its position and the positions of the other items in the list.
...
While the findIndex logic function is interesting in itself, in the context of a study of the animation side it is an implementation details and won't be explained but you can find it below.
